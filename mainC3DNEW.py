...
# Utility functions (modified for RGB only)
...
def plot_results(train_losses, val_losses, val_accuracies, val_f1_scores, val_precisions, val_recalls):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('training_results_c3d.png', format='png', dpi=300)
    plt.close()
...
def plot_class_distribution(labels, title):
    ...
    plt.savefig('class_distribution_c3d.png', format='png', dpi=300)
    plt.close()
...
def plot_confusion_matrix(y_true, y_pred, class_names, normalize=True, figsize=(10, 8), cmap='Blues', suffix=''):
    ...
    plt.savefig(f'confusion_matrix_c3d{suffix}.png', format='png', dpi=300)
    plt.close()
...
def plot_per_class_f1_scores(f1_scores, class_names, title, suffix=''):
    ...
    plt.tight_layout()
    plt.savefig(f'per_class_f1_scores_c3d{suffix}.png', format='png', dpi=300)
    plt.close()
...
val_split_size = 0.2
test_split_size = 0.3
height = 112  # Adjusted for C3D input requirements
width = 112
img_size = (height, width)
num_frames = 16  # C3D expects 16 frames
...
if create_csv_files:
    ...
    create_csv(train_videos, train_labels, os.path.join(csv_dir, "RGB_train_c3d"))
    create_csv(val_videos, val_labels, os.path.join(csv_dir, "RGB_val_c3d"))
    create_csv(test_videos, test_labels, os.path.join(csv_dir, "RGB_test_c3d"))

train_csv_path = os.path.join(csv_dir, "RGB_train_c3d.csv")
val_csv_path = os.path.join(csv_dir, "RGB_val_c3d.csv")
test_csv_path = os.path.join(csv_dir, "RGB_test_c3d.csv")
...
plot_class_distribution(class_counts, 'RGB Workout Video Dataset - Classes Distribution (C3D)')
...
# Simplified C3D Model for RGB only
