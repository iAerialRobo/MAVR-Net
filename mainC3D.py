...
if create_csv_files:
    create_csv(train_videos, train_labels, "trainC3D3")
    create_csv(val_videos, val_labels, "valC3D3")
    create_csv(test_videos, test_labels, "testC3D3")

train_csv_path = "/public/home/cit_fluo/znb/source/videoWork/trainC3D3.csv"
val_csv_path = "/public/home/cit_fluo/znb/source/videoWork/valC3D3.csv"
test_csv_path = "/public/home/cit_fluo/znb/source/videoWork/testC3D3.csv"
...
class WorkoutClassifierSmall(nn.Module):
    def __init__(self, num_classes=22):
        ...
        self.base_model.fc = nn.Identity()
        self.featureAttention = FFA.FFA(gps=3, blocks=19)
    ...
    def forward(self, x, flows):
        # print(x.shape)
        batch_size, num_frames, c, h, w = x.shape
        x = x.view(-1, c, h, w) # 240,3,128,128
        x = self.base_model(x)
        x = x.view(batch_size, num_frames, -1)
        x = torch.mean(x, dim=1)  # Temporal average
        x = self.fc1(x)
